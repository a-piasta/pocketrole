...
map,over:array[-21..1020,-21..1020]of char;
LABEL load;
label fuckit;
label fm;
begin
...
tempx:=x;
map[tempy,tempx]:=#1;
map[tempy-1,tempx]:=#254;
map[tempy+1,tempx]:=#254;
map[tempy,tempx-1]:=#254;
map[tempy,tempx+1]:=#254;
map[tempy-1,tempx-1]:=#254;
map[tempy-1,tempx+1]:=#254;
map[tempy+1,tempx-1]:=#254;
map[tempy+1,tempx+1]:=#254;
end;
...
wsp:=x*y;
seed:=random(200)+1;
if (map[y,x]=' ')and(wsp mod seed=0)and(wsp mod 3=0) then over[y,x]:=#7;
end;
end;
end;
//lake
...
begin
seed:=random(200)+1;
if (wsp mod 8=0) and (wsp mod seed=0) and (map[y,x]<>'~')and(map[y,x]<>#254)and (not(((x>492)and(x<508))and((y>492)and(y<508)))) and(map[y-1,x]<>'D')and(map[y+1,x]<>'D')and(map[y,x+1]<>'D')and(map[y,x-1]<>'D') then map[y,x]:='D';
end;
end;
end;
...
begin
if map[p,pp]=#1 then prawd:=prawd+1;
end;
end;
prawd:=prawd*100;
prawd:=300-prawd;
...
begin
if (map[p,pp]='~')or (map[p,pp]=#188)or (map[p,pp]=#187)or (map[p,pp]=#200)or (map[p,pp]=#201)or (map[p,pp]=#186)or (map[p,pp]=#205)then czd:=0;
end;
end;
...
wsp:=x*y;
seed:=random(200)+1;
if (map[y,x]=' ')and(wsp mod seed=0)and(wsp mod 3=0)and (not(((x>492)and(x<508))and((y>492)and(y<508)))) then over[y,x]:=#176;
end;
end;
//str castle
if 1=2 then
...
for x:=1 to 1000 do
begin
if (over[y,x]=#177) and (map[y,x]='~') then map[y,x]:=' ';
end;
end;
for y:=-15 to 1015 do
begin
for x:=-15 to 1015 do
begin
if map[y,x]='D' then over[y,x]:=#219;
end;
end;
for y:=1 to 1000 do
begin
...
wsp:=x*y;
seed:=random(2000)+1;
if (wsp mod 10000=0) and (wsp mod seed=0) and (over[y,x]=' ')and (map[y,x]=' ')then map[y,x]:=#245;
seed:=random(400)+1;
if (wsp mod 5000=0) and (wsp mod seed=0) and (over[y,x]=' ')and (map[y,x]=' ')then map[y,x]:='X';
seed:=random(300)+1;
if (wsp mod 2000=0) and (wsp mod seed=0) and (over[y,x]=' ')and (map[y,x]=' ')then map[y,x]:=#158;
seed:=random(200)+1;
if (wsp mod 800=0) and (wsp mod seed=0) and (over[y,x]=' ')and (map[y,x]=' ')then map[y,x]:='x';
seed:=random(200)+1;
if (wsp mod 500=0) and (wsp mod seed=0) and (over[y,x]=' ')and (map[y,x]=' ')then map[y,x]:=#2;
...
seed:=random(200)+1;
seet:=random(4)+1;
if ((wsp mod 243=0) and (wsp mod seed=0) and(map[y,x]=' '))or((map[y+1,x]=#254)and(wsp mod seet=0)and(map[y,x]=' '))or((map[y-1,x]=#254)and(wsp mod seet=0)and(map[y,x]=' '))or((map[y,x-1]=#254)and(wsp mod seet=0)and(map[y,x]=' '))
or((map[y,x+1]=#254)and(wsp mod seet=0)and(map[y,x]=' '))then map[y,x]:=#254;
end;
end;
end;
//portal
...
wsp:=x*y;
seed:=random(500)+1;
if (wsp mod 87=0)and(wsp mod seed=0)and(map[y,x]=' ')and(over[y,x]=' ')and (not(((x>492)and(x<508))and((y>492)and(y<508)))) then map[y,x]:=#207;
end;
end;
//menu
...
keypress:=readkey;
map[ty,tx]:=' ';
if (keypress='a') and ((map[ty,tx-1]=' ')or(map[ty,tx-1]=#176)) then tx:=Tx-1;
if (keypress='w') and (map[ty-1,tx]=#207) then
begin
repeat
tx:=random(800)+100;
ty:=random(800)+100;
...
if (keypress='s') and ((map[ty+1,tx]=' ')or(map[ty+1,tx]=#176)) then tY:=ty+1;
if (keypress='d') and ((map[ty,tx+1]=' ')or(map[ty,tx+1]=#176)) then tx:=tx+1;
if (keypress='w') and ((map[ty-1,tx]=' ')or(map[ty-1,tx]=#176)) then ty:=ty-1;
xb:=tx-16;
yb:=ty-10;
map[ty,tx]:=#5;
//mapdraw
...
if (map[p,pp]=#254) then
begin
textcolor(8);
textbackground(7);
end;
if (map[p,pp]=#3) then textcolor(12);
if (map[p,pp]='M') then textcolor(14);
if (map[p,pp]='X') or (map[p,pp]='x') or (map[p,pp]=#245) or (map[p,pp]=#158)then textcolor(4);
if (map[p,pp]=#2) then textcolor(11);
if (map[p,pp]=#1) then textcolor(14);
if (map[p,pp]=#207) then textcolor(13);
if map[p,pp]='D' then textcolor(6);
if (over[p,pp]=#176)and(map[p,pp]<>#5)then textcolor(16);
if over[p,pp]=' ' then write(map[p,pp])
else if map[p,pp]<>#5 then write(over[p,pp])
else if (map[p,pp]=#5) and (over[p,pp]=#177)then write(#178)
else if (map[p,pp]=#5) and (over[p,pp]=#176)then write(#5);
textcolor(black);
textbackground(lightgreen);
end;
write(#219);
...
for p:=1 to 36 do write(#219);
writeln();
textbackground(white);
writeln(#219,'X=',tx:3,' Y=',ty:3,'     Gold=',gold:13,#219);
textbackground(blue);
for p:=1 to 36 do write(#219);
...
fuckit:
//heal
if ((map[ty-1,tx]=#3)or(map[ty+1,tx]=#3)or(map[ty,tx+1]=#3)or(map[ty,tx-1]=#3)) and (keypress='u') then
begin
thp:=tmhp;
end;
//attackdisable
...
//sklep
if ((map[ty-1,tx]=#1)or(map[ty+1,tx]=#1)or(map[ty,tx+1]=#1)or(map[ty,tx-1]=#1)) and (keypress='u') then
begin
textbackground(blue);
textcolor(white);
potki:=kupione[8];
...
if (map[ty-1,tx]=#158) and (keypress='u') then
begin
gotoxy(18,10);
textbackground(green);
write(' ');
...
if (map[ty-1,tx]=#245) and (keypress='u') then
begin
gotoxy(18,10);
textbackground(green);
write(' ');
...
if cheat='portal' then map[ty-2,tx]:=#207;
...
//walka
if (map[ty-1,tx]=#2)or(map[ty+1,tx]=#2)or(map[ty,tx+1]=#2)or(map[ty,tx-1]=#2)then pcw:=random(2)+1;
bigrandom:=random(50);
if ((map[ty-1,tx]=#2) and (keypress='u'))or((map[ty-1,tx]=#2)and(pcw=1))or((map[ty+1,tx]=#2)and(pcw=1))or((map[ty,tx+1]=#2)and(pcw=1))or((map[ty,tx-1]=#2)and(pcw=1))or((bigrandom=1)and(attacdisable=0)) then
begin
if map[ty-1,tx]=#2then map[ty-1,tx]:=' ';
if map[ty+1,tx]=#2then map[ty+1,tx]:=' ';
if map[ty,tx+1]=#2then map[ty,tx+1]:=' ';
if map[ty,tx-1]=#2then map[ty,tx-1]:=' ';
potki:=kupione[8];
if ent<>10 then
begin
